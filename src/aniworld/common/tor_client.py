...
from aniworld.globals import IS_DEBUG_MODE, DEFAULT_USER_AGENT
...
class TorClient:
    ...
    def __init__(self, use_tor: bool = False):
        ...
        self.data_directory = os.path.join(tempfile.gettempdir(), f"aniworld_tor_{self.tor_port}")
    ...
    def start(self) -> bool:
        ...
        with self.lock:
            ...
            if not self._check_tor_installed():
                logging.error("Tor ist nicht installiert. Bitte installieren Sie Tor und versuchen Sie es erneut.")
                return False
            ...
            try:
                ...
                logging.info(f"Tor erfolgreich gestartet. IP: {self.ip_address}")
    ...
    def _get_hashed_password(self) -> Optional[str]:
        ...
        try:
            ...
            logging.error(f"Fehler beim Generieren des Hash-Passworts: {str(e)}")
            return None
    ...
    def _cleanup(self) -> None:
        ...
        try:
            ...
            logging.warning(f"Fehler beim Bereinigen des Tor-Datenverzeichnisses: {str(e)}")
        ...
        with self.lock:
            try:
                ...
                with Controller.from_port(address="127.0.0.1", port=self.control_port) as controller:
                    ...
                    # Kurz warten, damit die neue Identität erstellt werden kann
                    time.sleep(2)
                    ...
                    if new_ip and new_ip != old_ip:
                        ...
                        logging.info(f"Tor-IP gewechselt: {old_ip} -> {new_ip}")
                        return True
                    elif new_ip == old_ip:
                ...
                logging.error(f"Fehler beim Wechseln der Tor-Identität: {str(e)}")
                return False
    ...
    def setup_requests_session(self, session: Optional[requests.Session] = None) -> requests.Session:
    ...
    def make_request(self, 
        ...
        # Standardzeitüberschreitung auf 30 Sekunden setzen, wenn nicht anders angegeben
            ...
            try:
                ...
                if "Deine Anfrage wurde als Spam erkannt." in response.text:
                    if auto_retry and retry_count < max_retries:
                        logging.warning(f"IP blockiert. Wechsle zu neuer Tor-IP. Versuch {retry_count + 1}/{max_retries}")
                        if not self.new_identity():
                            logging.warning("Konnte IP nicht wechseln, versuche Tor neuzustarten")
                            self.restart_tor()
                        retry_count += 1
                        time.sleep(2)
                        continue
                    else:
                        logging.error("IP blockiert und auto_retry deaktiviert oder maximale Versuche erreicht")
                ...
                if auto_retry and retry_count < max_retries:
                    logging.warning(f"Verbindungsfehler: {str(e)}. Wechsle zu neuer Tor-IP. Versuch {retry_count + 1}/{max_retries}")
                    if not self.new_identity():
                        self.restart_tor()
                    retry_count += 1
                    time.sleep(2)
                ...
                if response and response.status_code in [403, 429, 503]:
                    if auto_retry and retry_count < max_retries:
                        logging.warning(f"HTTP-Fehler {response.status_code}: {str(e)}. Wechsle zu neuer Tor-IP. Versuch {retry_count + 1}/{max_retries}")
                        if not self.new_identity():
                            self.restart_tor()
                        retry_count += 1
                        time.sleep(2)
                    else:
                        logging.error(f"HTTP-Fehler {response.status_code}: {str(e)}")
                        return response, False
                else:
                ...
                logging.error(f"Unerwarteter Fehler bei HTTP-Anfrage: {str(e)}")
                return None, False
